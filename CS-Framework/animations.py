...
cmaps = ['magma', 'inferno', 'inferno_r', 'plasma', 'viridis', 'bone', 'afmhot', 'gist_heat', 'CMRmap', 'gnuplot',
...
def create_animation(header, cube_axis=np.array([]), cube=np.array([]), xlabel="", ylabel="", cblabel="", title="",
                     title_pad=0.0, vmin=None, vmax=None, output_video="dynamic_images.mp4", fps=30, interval=50,
                     repeat=False):
    ims = []
    num_ims = len(cube)
    if num_ims != 0:
        ...
        cv0 = cube[0]
        im = ax.imshow(cv0, origin='lower', aspect='equal', cmap='Spectral',
        ...
        ani = animation.FuncAnimation(fig, animate, frames=num_ims, interval=250, repeat=repeat, blit=False,
                                      repeat_delay=1000)
