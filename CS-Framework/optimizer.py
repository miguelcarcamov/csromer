...
class FISTA(Optimizer):
    def __init__(self, fx=None, gx=None, eta=2, L0=2, **kwargs):
    ...
    def run(self):
        ret, x = FISTA_algorithm(self.i_guess, self.F_obj.evaluate, self.fx.evaluate, self.gx.evaluate, self.fx.calculate_gradient, self.gx,
                       self.eta, self.maxiter, self.tol, self.L0, self.verbose)
        return ret, x
...
class ADMM(Optimizer):
    ...
    def run(self):
        x = self.i_guess
        converged, error = pmin.admm(x, prox_f=self.fx.calc_prox, step_f=None, prox_g=self.gx.calc_prox, L=None, e_rel=self.tol, max_iter=self.maxiter)
        #return
