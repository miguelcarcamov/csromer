...
def _norm2(arr):
    ...
    return float(maybe_compute(np.sum(np.abs(arr) ** 2)))
...
class TestDirectFourier1DE2E:
    ...
    def test_energy_conservation_adjoint_forward(self, dataset_direct_fourier, parameter_direct_fourier):
        ...
        inner = float(maybe_compute(np.real(np.vdot(a.ravel(), x.ravel()))))
        energy_data = _norm2(b)
    ...
    def test_rmtf_peak_normalization(self, dataset_direct_fourier, parameter_direct_fourier):
        ...
        op = DirectFourier1D(dataset=dataset_direct_fourier, parameter=parameter_direct_fourier)
        rmtf = op.RMTF()
        rmtf_np = np.asarray(maybe_compute(rmtf))
        n_phi = parameter_direct_fourier.n
        center = n_phi // 2
    ...
    def test_point_source_dirty_equals_1_over_rmtf_peak(self, dataset_direct_fourier, parameter_direct_fourier):
        ...
        data = op.forward(x)
        dataset_direct_fourier.data = data
        dirty = np.asarray(maybe_compute(op.dirty_spectrum(data)))
        rmtf = np.asarray(maybe_compute(op.RMTF()))
...
class TestGriddedFFT1DE2E:
    ...
    def test_point_source_1jy_dirty_value(self, dataset_gridded_fft, parameter_gridded_fft):
        """1 Jy point source (delta at center) -> dirty peak = 1 when K=1 (Jy per unit RMTF)."""
        op = GriddedFFT1D(dataset=dataset_gridded_fft, parameter=parameter_gridded_fft)
        n = parameter_gridded_fft.n
        center = n // 2
