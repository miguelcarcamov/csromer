...
def test_gridded_fft_point_source_peak(phi_gal, uniform_frequency_grid):
    ...
    assert _is_uniformly_spaced(
        gridded_source.lambda2
    ), "lambda² must be uniformly spaced for GriddedFFT1D"
    ...
    if abs(peak_phi - phi_gal) >= tolerance:
        ...
        print(
            f"  d_phi * d_lambda² = {d_phi_actual * d_lambda2:.6e}, expected π/N = {np.pi/n_channels:.6e}"
        )
