...
def create_animation(header, cube=np.array([]), xlabel="", ylabel="", cblabel="", title="", title_pad=55.0, output_cube="dynamic_images.mp4", fps=30, interval=50, repeat=False):
    ims = []
    num_ims = len(cube)
    if(num_ims != 0):
        fig = plt.figure()
        ...
        cv0 = cube[0]
        im = ax.imshow(cv0, origin='lower', aspect='equal', origin='lower', cmap='ocean_r', extent=[-axes[2], axes[2],-axes[3],axes[3]])
