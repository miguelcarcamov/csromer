...
class Plotter:
    ...
    def plot_overlay(self, dpi: np.int32 = 600, xlabel: str = None, ylabel: str = None, sigma_list: list = None,
        ...
        add_number(ax=ax, ra=173.498 * un.deg, dec=48.941 * un.deg, frame="fk5", text="4", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.521 * un.deg, dec=49.106 * un.deg, frame="fk5", text="5", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.571 * un.deg, dec=49.152 * un.deg, frame="fk5", text="6", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.621 * un.deg, dec=48.951 * un.deg, frame="fk5", text="7", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.632 * un.deg, dec=49.049 * un.deg, frame="fk5", text="8", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.693 * un.deg, dec=48.956 * un.deg, frame="fk5", text="9", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.705 * un.deg, dec=49.077 * un.deg, frame="fk5", text="10", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.712 * un.deg, dec=49.203 * un.deg, frame="fk5", text="11", fp=fp, wcs=wcs )
        add_number(ax=ax, ra=173.762 * un.deg, dec=49.193 * un.deg, frame="fk5", text="12", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.803 * un.deg, dec=48.966 * un.deg, frame="fk5", text="13", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.933 * un.deg, dec=49.038 * un.deg, frame="fk5", text="14", fp=fp, wcs=wcs)
        add_number(ax=ax, ra=173.943 * un.deg, dec=48.921 * un.deg, frame="fk5", text="15", fp=fp, wcs=wcs)
