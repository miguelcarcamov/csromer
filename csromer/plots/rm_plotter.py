...
class RMPlotter:
    def __init__(self, rm_image: Union[fits.PrimaryHDU, fits.HDUList, str] = None,
                 ...
                 total_intensity_nsigma: float = None, use_latex: bool = False):
    ...
    def get_lims(self, wcs: WCS = None):
        ...
        left_corner = SkyCoord(ra=self.center_coord.ra - radius_x, dec=self.center_coord.dec - radius_y)
        right_corner = SkyCoord(ra=self.center_coord.ra + radius_x, dec=self.center_coord.dec + radius_y)
    ...
    def plot(self, dpi=600, colorbar_label: Union[str, List[str]] = "", savefig: bool = False,
        ...
        if self.rm_image_error is not None and self.pol_fraction_image is not None:
            ...
            ax2.contour(total_intensity_data, transform=ax2.get_transform(wcs_total_intensity), levels=I_contours,
                        colors='black', alpha=0.6, linewidths=0.1)
            ...
            ax3.contour(total_intensity_data, transform=ax3.get_transform(wcs_total_intensity), levels=I_contours,
                        colors='black', alpha=0.6, linewidths=0.1)
            ...
            ax.contour(total_intensity_data, transform=ax.get_transform(wcs_total_intensity), levels=I_contours,
                       colors='black', alpha=0.6, linewidths=0.1)
