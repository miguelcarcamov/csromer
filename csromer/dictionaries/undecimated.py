...
class UndecimatedWavelet(Wavelet):
    ...
    def decompose(self, x):
        ...
        if signal_size and (signal_size % array_size) != 0:
            print("Your signal length is not multiple of 2**" +
                  str(self.level) + ". Padding array...")
            ...
            if self.mode is None:
                ...
                x_copy = pywt.pad(x=x_copy,
                                  pad_widths=(0, self.pad_width),
                                  mode=self.mode)
    ...
    def reconstruct(self, input_coeffs):
        ...
        if self.pad_width is not None:
            # Undo padding
            signal_from_coeffs = signal_from_coeffs[0:len(signal_from_coeffs) -
                                                    self.pad_width]
            self.pad_width = None
    ...
    def reconstruct_complex(self, input_coeffs):
        ...
        if self.append_signal:
            ...
            coeffs_re = pywt.array_to_coeffs(coeffs.real,
                                             self.coeff_slices[0],
                                             output_format="wavedec")
            coeffs_im = pywt.array_to_coeffs(coeffs.imag,
                                             self.coeff_slices[1],
                                             output_format="wavedec")

        else:
            coeffs_re = pywt.array_to_coeffs(input_coeffs.real,
                                             self.coeff_slices[0],
                                             output_format="wavedec")
            coeffs_im = pywt.array_to_coeffs(input_coeffs.imag,
                                             self.coeff_slices[1],
                                             output_format="wavedec")
