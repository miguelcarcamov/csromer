...
class FISTABacktracking(LineSearcher):
    ...
    def _get_proximal(self):
        ...
        non_diff = [
            t for t in self.objective_function.F if not getattr(t, "is_differentiable", True)
        ]
        if len(non_diff) == 0:
    ...
    def _g_at(self, x_array) -> float:
        ...
        return self.objective_function.calculate_function(x_array, nondifferentiable_only=True)
    ...
    def _compute_Q_L(self, x_k, y_k, f_y, grad_y, L) -> float:
        ...
        norm_sq = float(norm_sq.compute()) if hasattr(norm_sq,
                                                      "compute") else float(np.real(norm_sq))
    ...
    def search(self, x: Parameter, **kwargs) -> Tuple[float, float]:
        ...
        y_k = np.asarray(x.data.compute()) if hasattr(x.data,
                                                      "compute") else np.array(x.data, copy=True)
        grad_f_y = self.objective_function.dphi
        if grad_f_y is None:
            raise ValueError("objective_function.dphi must be set (call calculate_gradient)")
        grad_f_y = np.asarray(grad_f_y.compute()) if hasattr(grad_f_y,
                                                             "compute") else np.asarray(grad_f_y)
        ...
        for _ in range(self.max_iter):
            ...
            x_k_candidate = np.asarray(
                x_k_candidate.compute()
            ) if hasattr(x_k_candidate, "compute") else np.asarray(x_k_candidate)
            full_F = self.objective_function.evaluate(x_k_candidate)
            full_F = float(full_F.compute()
                           ) if hasattr(full_F, "compute") else float(np.asarray(full_F).item())
